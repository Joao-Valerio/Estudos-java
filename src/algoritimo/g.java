...
public class g {
	...
	public static int[] orgCrescente(int[] vetor) {
		for (int z = 0; z < vetor.length - 1; z++) {
			int mIndex = z;
			for (int x = z + 1; x < vetor.length; x++) {
				if (vetor[x] < vetor[mIndex]) {
					mIndex = x;
				}
			}
			int temp = vetor[mIndex];
			vetor[mIndex] = vetor[z];
			vetor[z] = temp;
		}
	...
	public static void buscaEspec(int[] vetor) {
		...
		if (ordenar.equalsIgnoreCase("ordenar") || ordenar.equalsIgnoreCase("sim")) {
			for (int z = 0; z < vetor.length - 1; z++) {
				int mIndex = z;
				for (int x = z + 1; x < vetor.length; x++) {
					if (vetor[x] < vetor[mIndex]) {
